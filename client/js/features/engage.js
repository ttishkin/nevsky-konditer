...
(function () {
  ...
  function injectNotifs(app) {
    ...
    card.style.cssText =
      "margin:0 16px 4px;padding:14px 16px;border-radius:16px;background:var(--card2);border:1px solid var(--sep)";
    var body = list.length
      ? list
          .slice(0, 5)
          .map(function (n) {
            return (
              '<div style="padding:7px 0;border-bottom:1px solid var(--sep);font-size:13px">🔔 ' +
              esc(n.message) +
    ...
    card.innerHTML =
      '<div style="font-size:15px;font-weight:700;margin-bottom:6px">Уведомления' +
  ...
  function injectReminder(app) {
    ...
    b.setAttribute("data-t", "diary");
    b.style.cssText =
      "margin:8px 16px;padding:12px 14px;border-radius:14px;background:rgba(199,249,75,.10);border:1px solid rgba(199,249,75,.28);font-size:13px;cursor:pointer";
    b.innerHTML =
      '📔 Отметьте съеденное в дневнике сегодня. <b style="color:var(--acc)">Открыть ›</b>';
