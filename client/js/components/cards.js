...
function icon(name, col) {
  ...
  var m = {
    ...
    diary:
      '<path d="M4 5a2 2 0 0 1 2-2h11l3 3v13a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2z"/><path d="M9 8h7M9 12h7M9 16h4"/>',
...
function thumb(p, h) {
  ...
  var bdg = p.hit
    ...
    : p.nov
      ...
      : "";
  var bg = u
    ? "linear-gradient(160deg,#FFFFFF,#ECECF0)"
    : "radial-gradient(120% 120% at 72% 18%," + hexA(ct.c, 0.3) + ",#14161d 62%)";
  var vis = u
    ? '<img class="pimg" src="' +
      u +
      '" loading="lazy" alt="" onerror="imgFallback(this)"><span class="gl gf" style="display:none">' +
      ct.gl +
      "</span>"
    : '<span class="gl gf">' + ct.gl + "</span>";
...
function matchFilter(p) {
  ...
  if (S.query) {
    ...
    if (p.n.toLowerCase().indexOf(q) < 0 && p.cat.toLowerCase().indexOf(q) < 0) return false;
  }
  return true;
}
function pcardHTML(p) {
  return (
    ...
    " ккал</span><br>" +
    priceHTML(p) +
    '</span><button class="qadd" data-act="addcart" data-id="' +
    p.id +
    '" aria-label="В корзину">+</button></div></div>'
  );
}
