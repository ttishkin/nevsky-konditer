...
const FREE_FROM = 1000, DELIVERY = 199, EARN_PCT = 5, MAX_SPEND_PCT = 30;
exports.create = (body, user) => {
  ...
  const line = items.map((it) => {
    ...
    return { productId: p.id, qty: Math.max(1, Number(it.qty != null ? it.qty : it.q) || 1), price: p.price };
  });
  const subtotal = line.reduce((s, l) => s + l.price * l.qty, 0);
  const delivery = subtotal >= FREE_FROM ? 0 : DELIVERY;
  let promoDiscount = 0, promoCode = null;
  if (body.promoCode) {
    const pr = promoRepo.byCode(String(body.promoCode).toUpperCase());
    if (pr && pr.active) { promoDiscount = Math.round((subtotal * pr.discountPercent) / 100); promoCode = pr.code; }
  }
  let bonusSpent = 0, bonusEarned = 0;
  if (user) {
  ...
  const order = orderRepo.create(
    { userId: user ? user.id : null, no, total, delivery, bonusEarned, bonusSpent, promoCode, status: "new", addressId: body.addressId },
    line
  );
  if (user) {
    if (bonusSpent) bonusRepo.add({ userId: user.id, orderId: order.id, amount: -bonusSpent, type: "spend" });
    if (bonusEarned) bonusRepo.add({ userId: user.id, orderId: order.id, amount: bonusEarned, type: "earn" });
    userRepo.addPoints(user.id, bonusEarned - bonusSpent);
  }
  order.subtotal = subtotal; order.promoDiscount = promoDiscount;
  return order;
};
