...
exports.list = (user, day) =>
  diaryRepo.byUserDay(user.id, day || new Date().toISOString().slice(0, 10));

exports.add = (user, body) => {
  if (!body.productId && !body.name) throw bad("Нужен productId или name");
  const qty = Math.max(1, Number(body.qty) || 1);
  let grams = body.grams || null,
    kcal = body.kcal || null;
  if (body.productId) {
    const p = productRepo.byId(body.productId);
    if (p) {
      grams = grams || p.g * qty;
      kcal = kcal || Math.round((p.kcal * grams) / 100);
    }
  }
...
exports.remove = (user, id) => {
  const r = diaryRepo.remove(id, user.id);
  if (!r.changes) throw bad("Запись не найдена", 404);
  return { ok: true };
};
