...
const pub =
  "id,email,name,sex,age,height,weight,activity,goal,kcal_norm AS kcalNorm,points,role,created_at AS createdAt";
exports.create = (u) => {
  const r = db
    .prepare(
...
exports.addPoints = (id, delta) =>
  db.prepare("UPDATE users SET points = MAX(0, points + ?) WHERE id=?").run(delta, id);
exports.updateProfile = (id, p) => {
  db.prepare(
    "UPDATE users SET name=?,sex=?,age=?,height=?,weight=?,activity=?,goal=?,kcal_norm=? WHERE id=?"
