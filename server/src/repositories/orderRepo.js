...
exports.create = (o, items) => {
  ...
  try {
    const r = db
      .prepare(
        "INSERT INTO orders (user_id,no,total,delivery,bonus_earned,bonus_spent,promo_code,status,address_id) VALUES (?,?,?,?,?,?,?,?,?)"
      )
      .run(
        o.userId || null,
        o.no,
...
exports.byId = (id) => {
  ...
  if (o)
    o.items = db
      .prepare("SELECT product_id AS productId, qty, price FROM order_items WHERE order_id=?")
      .all(id);
  return o;
};
