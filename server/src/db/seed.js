...
function seed() {
  ...
  try {
    ...
    const insP = db.prepare(
      "INSERT OR REPLACE INTO products (id,category_id,name,kcal,protein,fat,carb,grams,price,sostav,benefit,is_hit,is_novelty) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)"
    );
    ...
    const insPromo = db.prepare(
      "INSERT OR IGNORE INTO promo_codes (code,discount_percent,active) VALUES (?,?,1)"
    );
    insPromo.run("ЗОЖ10", 10);
    insPromo.run("ДРУГ500", 0);
    if (db.prepare("SELECT COUNT(*) c FROM pickup_points").get().c === 0) {
      const insPvz = db.prepare(
        "INSERT INTO pickup_points (name,address,lat,lng) VALUES (?,?,?,?)"
      );
      insPvz.run(
        "Невский Кондитер — Невский пр.",
        "Санкт-Петербург, Невский пр., 100",
        59.93,
        30.36
      );
      insPvz.run(
        "Невский Кондитер — Лиговский",
        "Санкт-Петербург, Лиговский пр., 50",
        59.92,
        30.35
      );
    }
    if (!db.prepare("SELECT id FROM users WHERE email=?").get("admin@nk.ru")) {
      db.prepare("INSERT INTO users (email,password_hash,name,role,points) VALUES (?,?,?,?,?)").run(
        "admin@nk.ru",
    ...
    if (!db.prepare("SELECT id FROM users WHERE email=?").get("demo@nk.ru")) {
      db.prepare(
        "INSERT INTO users (email,password_hash,name,sex,age,height,weight,activity,goal,kcal_norm,points) VALUES (?,?,?,?,?,?,?,?,?,?,?)"
      ).run(
        "demo@nk.ru",
...
if (require.main === module) {
  ...
  require("../utils/logger").info(
    "Сидинг:",
    JSON.stringify({ categories: c("categories"), products: c("products"), users: c("users") })
  );
}
